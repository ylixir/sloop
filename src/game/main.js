		...
		function() {
			...
			game.createClass('ScurvyDog', {
				...
				remove : function() {
					if (Math.random(0, 1) < chanceToDie) {
						...
						game.scene.addTimer(liveCheckTime, this.remove
								.bind(this))
				},
				collide : function() {
					this.bearing.x *= -1
			...
			game.createScene('Main', {
				...
				init : function() {
					...
					var xStart = game.width / 2;
					var yStart = game.height / 2;
					for (var i = 0; i < 10; i++) {
						grap.drawCircle(xStart, yStart, 5);
						xStart += 100;
						yStart += 100;
					}
					xStart = game.width / 2;
					yStart = game.height / 2;
					for (var i = 0; i < 10; i++) {
						grap.drawCircle(xStart, yStart, 5);
						xStart -= 100;
						yStart -= 100;
					...
					walls.static = true;
					walls.addShape(new game.Rectangle(100, game.height,
							game.width, 0))
					walls.addShape(new game.Rectangle(game.width, 100, 0,
							game.height))
					...
					this.addTimer(liveCheckTime, function() {
						if (Math.random(0, 1) < 1 - chanceToDie) {
							var pirate = new game.ScurvyDog(
									game.width.random(), game.height.random(),
									Math.random(-100, 100), Math.random(-100,
											100));
							// pirate.poop();
						}
